    ...
    def choose_method():
        ...
        while True:
            choose_msg = input('\nChoose one of methods (print number) and press Enter \n'
                               '1 - URL_UPDATED: index post to google search\n'
                               '2 - URL_DELETED: remove post from google search results\n')
            if '1' in choose_msg:
                method = 'URL_UPDATED'
                break
...
if __name__ == '__main__':
    g_index = GoogleIndexationAPI('service_account.json', 'urls.txt')
